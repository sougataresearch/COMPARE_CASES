...
`python 01_main.py`. Handles hardware bring-up, guided camera setup,
retried/verified image acquisition, crash-safe checkpointing (`--resume`),
CSV logging, and a final report. See its own `README.md` for full operator
instructions and a function-by-function reference.

### `Measuremt_ script/continous_rotation/`

The entry point for **4×4 continuous rotation**, where both QWPs spin
continuously at a fixed revolution ratio (classically 1:5 — see that
folder's README) instead of stepping through discrete angles. Hardware
bring-up, camera setup, and plan/config persistence are implemented and
runnable today (including dry-run); the acquisition loop itself
...
that module's docstring for the two options.
